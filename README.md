...
- Project Map panel with saved domain, owner, and roadmap filter presets for dependency tracing, open risks, roadmap stages, decisions, owners, and domains
- One-click "Sync to Chat" summary copied to your clipboard
- JSON export/import for full-state handoff or migration, plus markdown export presets for reviews
...
Click `Settings` in the header to edit the local project title, work item label, cycle label, and import field aliases. Settings are saved to ignored local `config.json` so forks can keep reusable defaults in Git while each user keeps private local wording on their machine.
