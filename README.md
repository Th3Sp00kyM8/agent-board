...
Click `Settings` in the header to edit the local project title, work item label, cycle label, import field aliases, and custom markdown export presets. Settings includes import alias presets for Default, GitHub Issues, Jira, and Linear-style schemas, plus a searchable starter gallery and form-based custom export preset builder for adding, editing, deleting, copying, and downloading reusable markdown templates. The starter gallery includes category filters, favorites, last-used sorting, and preview examples for executive status, release readiness, risk watch, decision review, and planning intake report templates before adding them. Settings are saved to ignored local `config.json` so forks can keep reusable defaults in Git while each user keeps private local wording on their machine.
